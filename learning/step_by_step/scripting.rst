...
In that sense, Mole has taken some useful design decisions towards that
...
different design pattern to MVC. Mole promises efficiency at the
expense of dropping the MVC habits, which are replaced by the *scenes as
a complement* pattern.

Mole also uses the `extend <http://c2.com/cgi/wiki?EmbedVsExtend>`__
...
:ref:`doc_moscript` is a dynamically typed scripting language to fit
inside Mole. It was designed with the following goals:
...
the replacement of a MoScript class with a C++ class without altering
the rest of the game.

Scripting a scene
-----------------

Before continuing, please make sure to read the :ref:`doc_moscript` reference.
...
the language, class name, etc. MoScript does not use class names in
...
For this, a function exists that is probably the one most used by Mole
