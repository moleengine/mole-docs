...
scene system is not even a core component of Mole, as it is possible to
...
The way Mole works internally is as follows. There is the
...
level and when making games in Mole, writing your own MainLoop does not
even make sense.

SceneTree
---------

One of the ways to explain how Mole works, is that it's a high level
...
Most node operations in Mole, such as drawing 2D, processing or getting
...
#. Every node of the newly added scene, will receive the "enter_tree"
   notification ( _enter_tree() callback in MoScript) in top-to-bottom
   order.
#. An extra notification, "ready" ( _ready() callback in MoScript) is
...
#. When a scene (or part of it) is removed, they receive the "exit
   scene" notification ( _exit_tree() callback in MoScript) in
   bottom-to-top order
