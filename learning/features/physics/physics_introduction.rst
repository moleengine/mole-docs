...
The base collidable object in Mole's 2D world is a
...
Mole will return correct collision and collision info from the
...
As seen before in the collide functions, 2D shapes in Mole can be
...
Introducing... Mole's physics engine!
--------------------------------------

To solve all these problems, Mole has a physics and collision engine
...
Mole uses a built-in custom 2D physics engine that is designed to
...
transform. This means that all collisionable objects in Mole can use
...
Areas in Mole physics have three main roles:
...
need tweaking which is the "cell_size". Mole 2D physics engine used by
...
To solve this, Mole has a fixed process callback, which is like process
