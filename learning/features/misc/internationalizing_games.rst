...
Mole offers many tools to make this process more straightforward, so
...
It is also possible to instruct Mole to open alternative versions of
...
Mole has a server for handling the low level translation management
...
Language can be tested when running Mole from command line. For
example, to test a game in french, the following arguments can be
supplied:

::

   c:\MyGame> mole -lang fr
