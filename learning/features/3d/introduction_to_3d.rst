...
Mole uses very similar APIs for 2D and 3D. Most nodes are the same and
...
exchange file format in order to be imported in Mole (3D formats are
not as standardized as images).

DCC-created models
------------------

There are two pipelines to import 3D models in Mole. The first and most
...
will be updated often, Mole provides a special node,
...
While Mole packs a powerful 2D engine, many types of games use 2D in a
...
Mole provides a :ref:`WorldEnvironment <class_WorldEnvironment>`
...
Mole uses the `metric <http://en.wikipedia.org/wiki/Metric_system>`__
...
scale (set your DCC to metric). Mole allows scaling post-import and,
...
Mole. As many as desired can be added (as long as performance allows). Shadow
