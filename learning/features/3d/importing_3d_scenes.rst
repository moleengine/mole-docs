...
object placement, full scene animations, etc. In Mole, given the node
...
-  Importing full scenes. Entire scenarios can be created and updated in
   the 3D DCC and imported to Mole each time they change, then only
...
distributed with a more restrictive licensing terms than Mole. The plan
...
Mole provides a `Python
Plugin <https://github.com/moleengine/collada-exporter>`__
...
are usually just PNG or JPG files. Mole will convert them to video
...
imported, Mole will convert them to what they are meant to be.
...
and how it is imported to Mole:
...
Originally Mole detected this based on whether if the source texture
...
the 3D scene importer in Mole that this material will use the alpha
...
-  It should lay on the surface of the room (this doesn't need to be
   super exact, just make it as close as you can by eye and Mole will
   adjust it)
...
not drawn). Mole supports this option per Material, not per Geometry.
...
added data or data that can't be edited inside Mole will be updated.
...
-  Portal and Room shapes will be replaced by the ones from the updated
   scene.
-  If the user moved a node inside Mole, the transform will be kept. If
