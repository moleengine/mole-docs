...
*MoScript* is a high level, dynamically typed programming language used to
...
to be optimized for and tightly integrated with Mole Engine, allowing great
flexibility for content creation and integration.

History
~~~~~~~

Initially, Mole was designed to support multiple scripting languages
(this ability still exists today). However, only MoScript is in use
...
more optimally make use of Mole's particular architecture:

-  Mole embeds scripts in nodes. Most languages are not designed with
   this in mind.
-  Mole uses several built-in data types for 2D and 3D math. Script
   languages do not provide this, and binding them is inefficient.
-  Mole uses threads heavily for lifting and initializing data from the
   net or disk. Script interpreters for common languages are not
   friendly to this.
-  Mole already has a memory management model for resources, most
...
The result of these considerations is *MoScript*. The language and
interpreter for MoScript ended up being smaller than the binding code itself
...
here's a simple example of how MoScript looks.
...
read this tutorial: :ref:`doc_moscript_more_efficiently`.

Language
--------

In the following, an overview is given to MoScript. Details, such as which
...
A variable in MoScript can be assigned to several built-in types.
...
MoScript supports :ref:`format strings aka printf functionality
<doc_moscript_printf>`.
...
Starting with Mole 2.1, indices may be negative like in Python, to count from the end.
...
MoScript arrays are allocated linearly in memory for speed. Very
...
Note however that like any MoScript identifier, keys written in this form cannot
start with a digit.
...
assign to it::
    ...
    d["Mole"] = 3.01 # add String "Mole" as a key and assign the value 3.01 to it
...
Contrary to Python, functions are *not* first class objects in MoScript. This
...
::
    ...
    # saved as a file named myclass.mo
...
::
    ...
    extends "somefile.mo".SomeInnerClass
...
::
    ...
    const enemy_class = preload("enemy.mo")
...
Classes stored as files are treated as :ref:`resources <class_MoScript>`. They
must be loaded from disk to access them in other classes. This is done using
either the ``load`` or ``preload`` functions (see below). Instancing of a loaded
class resource is done by calling the ``new`` function on the class object::

    # Load the class resource when calling load()
    var MyClass = load("myclass.mo")

    # Preload the class only once at compile time
    var MyClass2 = preload("myclass.mo")
...
For this, MoScript provides a *setter/getter* syntax using the ``setget`` keyword.
...
instance. MoScript supports creation of built-in Mole signals.
Declaring a signal in MoScript is easy using the `signal` keyword.
...
MoScript offers support for `coroutines <https://en.wikipedia.org/wiki/Coroutine>`_
...
references pile up. For this, MoScript has the ``onready`` keyword, that
