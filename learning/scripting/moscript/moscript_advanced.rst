.. _doc_moscript_more_efficiently:

MoScript more efficiently
=========================

About
-----

This tutorial aims to be a quick reference for how to use MoScript more
...
MoScript is a Dynamically Typed language. As such, it's main advantages
are that:
...
This, translated to reality, means that Mole+MoScript are a combination
...
entire game in MoScript and add small bits of C++ in the areas that need
a performance boost.
...
In MoScript, only base types (int, float, string and the vector types)
...
MoScript dictionaries resemble python dictionaries, it also supports Lua
...
doesn't exist (like Objective C), but MoScript is more strict, so
