...
entirely with MoScript and standard scenes, without even reloading the editor.
...
configuration and a custom MoScript with the functionality.
...
file. Mole is not able (yet) to open text files besides scripts, so this must
...
``plugin.cfg``::
    ...
    name="My Custom Node"
    description="A custom node made to extend the Mole Engine."
    author="Your Name Here"
    version="1.0"
    script="custom_node.mo"
...
Open the script editor (F3) and create a new MoScript file called
``custom_node.mo`` inside the ``my_custom_node`` folder. This script is special
...
default MoScript template from your file and replace it with the following
structure:
...
That's it for our basic button. You can save this as ``button.mo`` inside the
...
dialog. For that, change the ``custom_node.mo`` script to the following::
    ...
    func _enter_tree():
        ...
        add_custom_type("MyButton", "Button", preload("button.mo"), preload("icon.png"))
...
your favorite text editor add the following content to it::
    ...
    author="Your Name Here"
    version="1.0"
    script="custom_dock.mo"

Then create the script ``custom_dock.mo`` in the same folder. Fill with the
...
such dock. This is nothing more than a standard Mole scene. So you can create
...
many nice ways. Many functions can be added to editor on the fly with MoScript,
it is a powerful way to create special editors without having to delve into C++
modules.

You can make your own plugins to help you and also share them in Mole's Asset
